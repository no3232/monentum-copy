...
async function welcome() {
  ...
  console.log(
    chalk.white(
      "이 스크립트는 Moment Chrome Extension을 설정하는 과정을 안내합니다.\n"
    )
  );
...
async function setupGoogleCloudProject() {
  ...
  console.log(
    chalk.bold.yellow("📋 Step 1: Google Cloud Console 프로젝트 생성\n")
  );
  ...
  console.log(
    chalk.gray('3. 프로젝트 이름 입력 (예: "Moment Extension") → "만들기"\n')
  );
...
async function setupUnsplash() {
  ...
  console.log(
    chalk.gray("   - Description: New tab extension with Unsplash backgrounds")
  );
...
async function updateManifestClientId(clientId) {
...
async function createEnvFile(clientId, unsplashKey) {
  ...
  // manifest.json에 Client ID 추가
  updateManifestClientId(clientId);

  // .env 파일 생성 (Unsplash만)
  const envContent = `# Unsplash Access Key
# https://unsplash.com/developers 에서 생성
VITE_UNSPLASH_ACCESS_KEY=${unsplashKey}

# 참고: Google OAuth Client ID는 manifest.json의 oauth2.client_id에 설정됩니다.
`;
  ...
  spinner.succeed(
    chalk.green("manifest.json 및 .env 파일이 성공적으로 업데이트되었습니다!")
  );
  await wait(1000);
}
...
async function chromeExtensionInstructions(built) {
  ...
  if (!built) {
    console.log(
      chalk.yellow("⚠️  먼저 프로젝트를 빌드해주세요: npm run build\n")
    );
  }
  ...
  console.log(
    chalk.gray("5. 로드된 확장프로그램의 ID를 복사 (예: abcdefgh...)")
  );
...
async function finalOAuthUpdate(extensionId) {
  ...
  console.log(
    chalk.bold.yellow("🔄 Step 8: OAuth 클라이언트에 애플리케이션 ID 등록\n")
  );
  ...
  console.log(chalk.gray('3. "애플리케이션 ID" 필드에 다음 값 입력:'));
  console.log(chalk.cyan(`   ${extensionId}`));
  console.log(chalk.gray('4. "저장" 클릭\n'));
  console.log(
    chalk.green(
      "💡 Chrome Identity API를 사용하므로 리디렉션 URI 설정은 필요 없습니다.\n"
    )
  );
  ...
  const { completed } = await inquirer.prompt([
    {
      type: "confirm",
      name: "completed",
      message: "OAuth 클라이언트에 애플리케이션 ID를 등록했습니까?",
      default: false,
    },
  ]);
